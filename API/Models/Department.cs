...
Description : <โมเดลข้อมูลภาควิชา>
=============================================
*/
...
namespace API.Models
{
  public class Department
  {
    ...
    {
      DataSet ds = iUtil.ExecuteCommandStoredProcedure(iUtil.infinityConnectionString, "sp_acaGetListDepartment",
        new SqlParameter("@xmlData", new StringBuilder().AppendFormat("<table><row><facultyId>{0}</facultyId></row></table>", facultyId).ToString()));

      return ds;
    }
        
    public static DataSet GetData(string departmentId)
    {
      DataSet ds = iUtil.ExecuteCommandStoredProcedure(iUtil.infinityConnectionString, "sp_acaGetDepartment",
        new SqlParameter("@id", departmentId));

      return ds;
    }

    public static DataSet SetData(string method, List<Department> data, dynamic account)
    {
      ...
      foreach (var d in data)
      {
        ...
        (d.depType != null ? ("<depType>" + d.depType + "</depType>") : String.Empty) +                
      ...
      DataSet ds = iUtil.ExecuteCommandStoredProcedure(iUtil.infinityConnectionString, "sp_acaSetDepartment",
